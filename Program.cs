...
namespace Training {
   ...
   internal class Program {
      ...
      /// <summary> CheckYes if the value guessed is correct and exit program if it is. </summary>
      /// <param name="prompt"> User Input </param>
      static bool CheckYes (string prompt) {
      ...
      static void GuessNumberBinaryMSB_LSB () {
         do {
            ...
            if (CheckYes (prompt) || prompt is "high") high = bGuess;
            else low = bGuess;
            bGuess = (low + high) / 2;
         } while (high > low);
      }
